...
(function($) {
	...
	*/
		...
		var itemLength = object.items.length;

		if ( itemLength > 0) {
			//if ( object.items[itemLength - 1].url === data.url ) {
			if (get_obj_by_key_value(object.items, "url", data.url).length) {
				return false;
			} else {
	...
	$(function() {
		...
		if (target.length) {
			...
			if (json){
				...
				if ( localStorage.getItem("access_data") ) {
					target.after("<div class='clearStorage'>Clear LocalStorage Data</div>");
				}
